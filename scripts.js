/*
  --------------------------------------------------------------------------------------
  Chamada da função para carregamento inicial dos tipos de pet
  --------------------------------------------------------------------------------------
*/
getTypes()
...
const getTypes = async () => {
  let url = 'http://127.0.0.1:5000/tipos';
  fetch(url, {
    method: 'get',
  })
...
const getList = async () => {
  ...
  fetch(url, {
    ...
    .then((response) => response.json())
    .then((data) => {
      data.pets.forEach(item => insertList(item.nome, item.quantidade, item.valor))
    })
    .catch((error) => {
      console.error('Error:', error);
    });
